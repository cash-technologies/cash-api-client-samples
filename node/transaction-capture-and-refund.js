...
const captureCharge = (transactionId, body, apiKey) => {
  ...
  const url = `https://sandbox.api.holacash.mx/v2/transaction/capture/${transactionId}`;
...
const refundCharge = (transactionId, body, apiKey) => {
  ...
  const url = `https://sandbox.api.holacash.mx/v2/transaction/refund/${transactionId}`;
  return new Promise((resolve, reject) => {
    const request = https
      .request(url, options, response => {
        let data = "";
...
if (require.main === module) {
  const {
    ...
    createCharge,
    ANTIFRAUD_METADATA
  } = require("./transaction-w-credit-card");
