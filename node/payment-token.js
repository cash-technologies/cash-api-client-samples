...
const createPaymentToken = (body, apiKey, antifraudMetadata) => {
  const antifraudMetadataBase64 = Buffer.from(
    JSON.stringify(antifraudMetadata)
  ).toString("base64");
  const options = {
    method: "POST",
    headers: {
  ...
  const url = "https://sandbox.api.holacash.mx/v2/tokenization/payment_token";
...
const getPaymentToken = (tokenId, apiKey) => {
  ...
  const url = `https://sandbox.api.holacash.mx/v2/tokenization/payment_token/${tokenId}`;
  return new Promise((resolve, reject) => {
    const request = https
      .request(url, options, response => {
        let data = "";
